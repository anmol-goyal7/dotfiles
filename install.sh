...
echo ""
echo "  5. Power button: the default is HandlePowerKey=poweroff, so tapping it"
echo "     at a screen that will not wake cleanly shuts the laptop down."
echo "     sudo install -Dm644 $DOTFILES/etc/systemd/logind.conf.d/10-power-key.conf \\"
echo "       /etc/systemd/logind.conf.d/10-power-key.conf"
