...
- **Red mode** — `SUPER+N`, or `red` / `blue` at a prompt. A fragment shader that drops the green and blue channels outright, so the screen goes fully red for night work. It hangs on `decoration:screen_shader`, which exists only at runtime, so every config reload silently drops it — leaving zen mode, leaving game mode, and above all `misc:disable_autoreload` being off, which makes Hyprland re-read its own config every time `hyprland.lua` or a module under `lua/` is saved. That last one is why the tint kept disappearing mid-edit with nothing to blame: `bin/red` kept a state file and a `reapply` verb, but only zen and game mode ever called it. `red watch`, started from `lua/startup.lua`, listens for `configreloaded` on the event socket and reapplies after every reload whoever caused it. It also clears the state file on the way up, since `$XDG_RUNTIME_DIR` outlives the compositor and a leftover `red` would otherwise reappear in a later session unasked.
...
├── etc/            scoped NOPASSWD rules the power scripts need (`sudoers.d/`),
│                and the logind drop-in that stops a power-button tap shutting
│                the laptop down (`systemd/logind.conf.d/`)
